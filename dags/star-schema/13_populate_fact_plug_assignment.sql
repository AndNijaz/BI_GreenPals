...
-- 13_populate_fact_plug_assignment_optimized.sql

-- 1. Kreiramo privremenu tabelu sa agregiranim assignmentima
...
SELECT
  ...
  SUM(EXTRACT(EPOCH FROM (COALESCE(end_time, CURRENT_TIMESTAMP) - start_time)) / 3600) AS usage_hours,
  MAX(updated_at) AS created_at
FROM cleaned.plug_assignments
GROUP BY plug_id, device_id, room_id, DATE(start_time);

-- 2. Ubacujemo u fact tabelu
