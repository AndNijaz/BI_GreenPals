...
def archive_company_devices_incremental():
    ...
    process = "scd2_incremental_archive_company_devices"
    source = "landing.company_devices"

    # Dohvati najnoviji updated timestamp iz arhive
    cur.execute("SELECT MAX(updated) FROM archive.company_devices")
    last_updated = cur.fetchone()[0] or datetime(2000, 1, 1)
    print(f"🔍 Arhiviramo company_devices ažurirane nakon: {last_updated}")

    # Dohvati company_devices iz landing novije od last_updated
    cur.execute("""
    ...
    for chunk in batch(rows):
            ...
            if not archive_rows:
                ...
                for row in archive_rows:
                    ...
                    if name != old_name or category != old_category:
                        # Zatvori stari red
                        cur.execute("UPDATE archive.company_devices SET end_date = %s WHERE id = %s", (now, archive_id))
                        # Ubaci novi red
                        cur.execute("""
                            INSERT INTO archive.company_devices (
                                start_date, end_date, process, source, updated,
        ...
        conn.commit()
        print(f"✅ Obrada batcha od {len(chunk)} company_devices završena.")

    cur.close()
    conn.close()
    print("🎉 Incremental SCD2 arhiviranje company_devices završeno.")

if __name__ == "__main__":
    archive_company_devices_incremental()
