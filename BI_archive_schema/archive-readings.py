...
def archive_readings_incremental():
    ...
    process = "scd2_incremental_archive_readings"
    source = "landing.readings"

    # Dohvati najnoviji updated timestamp iz arhive
    cur.execute("SELECT MAX(updated) FROM archive.readings")
    last_updated = cur.fetchone()[0] or datetime(2000, 1, 1)
    print(f"🔍 Arhiviramo readings ažurirane nakon: {last_updated}")

    # Dohvati readings iz landing novije od last_updated
    cur.execute("""
    ...
    for chunk in batch(rows):
        for reading_id, plug_id, ts, power_kwh, updated_at in chunk:
            # Provjeri da li postoji aktivni red za plug_id + timestamp
            cur.execute("""
                SELECT id, power_kwh
            ...
            if not archive_rows:
                ...
                for row in archive_rows:
                    ...
                    if float(power_kwh) != float(old_power):
                        # Zatvori stari red
                        cur.execute("UPDATE archive.readings SET end_date = %s WHERE id = %s", (now, archive_id))
                        # Ubaci novi red
                        cur.execute("""
                            INSERT INTO archive.readings (
                                start_date, end_date, process, source, updated,
                        ...
                        break
        conn.commit()
        print(f"✅ Obrada batcha od {len(chunk)} readings završena.")

    cur.close()
    conn.close()
    print("🎉 Incremental SCD2 arhiviranje readings završeno.")

if __name__ == "__main__":
    archive_readings_incremental()
